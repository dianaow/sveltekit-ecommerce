...
{#if $open}
   <button {...$close} use:close class="flex relative mx-2 p-2 items-center justify-center hover:bg-stone-200 rounded-md">
      ...
      {#if count > 0}
         <span class="z-50 absolute top-3 right-2 inline-flex items-center justify-center px-2 py-1 text-xs font-bold leading-none text-white transform translate-x-1/2 -translate-y-1/2 bg-indigo-600 rounded-full">
            {count}
         </span>
      {/if}
   </button>
         ...
         <div class="px-8 sm:px-12">
            ...
            <section aria-labelledby="summary-heading" class="border-t border-gray-200 bg-white sticky bottom-0 py-6">
               ...
               <form action="/checkout">
                  <button use:close type="submit" class="my-4 w-full items-center justify-center rounded-md border border-transparent bg-indigo-600 px-5 py-3 text-base font-medium text-white hover:bg-indigo-700">Checkout</button>
               </form>
               {/if}
