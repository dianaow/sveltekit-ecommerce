   ...
   <div class="lg:max-w-lg lg:self-end">
      ...
      <div class="mt-6">
         ...
         <div class="mt-4">
            ...
            <button type="button" class="uppercase whitespace-nowrap px-3 py-2 mr-2 mb-2 rounded-lg text-sm font-medium text-gray-700 border-4 border-indigo-600 bg-white hover:bg-white">
               {value}
            </button>
            {:else}
      ...
      <form action="/cart?/add" method="post" use:enhance={() => { return async ({ result }) => { if (result.type === 'success') { await invalidateAll() }}}}>
         ...
         <button type="submit" class="mt-6 w-full items-center justify-center rounded-md border border-transparent bg-indigo-600 px-5 py-3 text-base font-medium text-white hover:bg-indigo-700">
            Add to Cart
         </button>
      </form>
   </div>
   ...
   <div class="max-w-screen-lg lg:col-span-2">
      <div class="flex" aria-orientation="horizontal" role="tablist">
         ...
         <button type="button" on:click="{() => tab = 'other'}" class="{tab === 'other' ? 
            "whitespace-nowrap p-3 pr-4 mr-4 border-b-2 font-medium border-indigo-600 text-gray-800" : 
